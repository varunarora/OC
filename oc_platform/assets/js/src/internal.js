OC.internal = {
    ...
    standards: {
        init: function(){
            function fitPage(){
                $('.standards-play').height($(window).height());
                $('.standards-play .left-menu, .standards-play .middle-menu').height(
                    $('.standards-play').height() - $('.category-selection-bar').outerHeight() -
                    parseInt($('.standards-play .left-menu').css('padding-top'), 10));
            }

            fitPage(); $(window).resize(fitPage);

            OC.internal.standards.bindGradeCategoryClick();
        },

        bindGradeCategoryClick: function(){
            $('ul.category-grades li').click(function(event){
                $('.standards-play .middle-menu').addClass('loading');
                var categoryID = $(event.target).attr('id');

                $.get('/meta/api/get-nested-child-tags-from-category/' + categoryID  + '/',
                    function (response){
                        var i;
                        for (i = 0; i < response.tags.length; i++){
                            $('.standards-play .middle-menu').append(
                                response.tags[i].title + response.tags[i].description);
                        }
                        $('.standards-play .middle-menu').removeClass('loading');
                    }, 'json');
            });
        }
    }
...
var Grade = Backbone.Model.extend({ id: '', title: '', position: '' });
var Tag = Backbone.Model.extend({ id: '', title: '', position: '', description: '', mappings: '' });

var GradeSet = Backbone.Collection.extend({ model: Grade });
var TagSet = Backbone.Collection.extend({ model: Tag });
var GradeTags = Backbone.Collection.extend({ model: TagSet });
...
jQuery(document).ready(function ($) {
    ...
    /*** Setup standards play ***/
    OC.internal.standards.init();

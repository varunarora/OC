define(['jquery', 'underscore', 'backbone', 'core', 'dropzone'], function($, _, Backbone, OC, Dropzone){

// Initialize Group posts Model.
OC.Post = Backbone.Model.extend({
    // ID of the post.
    id: "",
...
OC.PostSet = Backbone.Collection.extend({
    model: OC.Post,
    ...
    init_mvc: function() {
        ...
        _.each(filterCheckboxes, function (filter) {
            $(filter).change(function () {
                repopulatePosts(this, OC.groups.postCollectionView);
            });
        });
        ...
        $('.filter-sort-option').click(function(event){
            if ($(event.target).hasClass('filter-sort-option-newest')){
                OC.groups.postSet.sortByNewest();
                OC.groups.postCollectionView.render();
            } else {
                OC.groups.postSet.sortByPopularity();
                OC.groups.postCollectionView.render();
            }
        });
        ...
        $('.discussion-board-filters-search input').keyup(function(event){
            ...
            var collectionToRender = new OC.PostSet(OC.groups.postSet.filter(function (post) {
                return post.get('body').toLowerCase().indexOf(currentInput.toLowerCase()) !== -1;
            }));

            // Recreate the view (clears previous view)
            OC.groups.postCollectionView.render(collectionToRender);
        });
...
OC.PostView = Backbone.View.extend({
    ...
    events: {
        ...
        'click .post-body-wrapper > .delete-button': OC.groups.bindPostDeleteButtons,
...
OC.PostCollectionView = Backbone.View.extend({
    render: function (newCollection) {
        ...
        // Create a new view object for each object in the collection and render it
            _.each(collectionToRender.models, function(item) {
                new OC.PostView({model: item}).render();
            });
        }
        this.revealView();
    },
...
function repopulatePosts(target, resourceCollectionView) {
    ...
    var collectionToRender = new OC.PostSet(OC.groups.postSet.reject(function (post) {
